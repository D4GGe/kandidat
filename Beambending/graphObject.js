function graphObj(type) {
    ...
    this.amount = 100;
    this.moveClose = function (beam) {
        var x = beam.x;
        var y = beam.y;
        var xMax = beam.width + x;
        var yMax = beam.height + y;

        if (y + beam.height / 2 <= self.y + self.yIn) {

            var newY = y + beam.height - self.yIn;
        ...
        if (self.x + self.xIn < x) {

            var newX = x - self.xIn;
        } else if (xMax < self.x + self.xIn) {

            var newX = xMax - self.xIn;
        } else {
